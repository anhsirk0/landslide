...
let make = () => {
  ...
  let routeCards = Route.appRoutes->Array.map(item => {
    <Route.Link
      to=item.path
      key=item.path
      className="col-span-12 md:col-span-4 xl:col-span-4 2xl:col-span-3 rounded-box overflow-hidden h-16 md:h-24 cursor-pointer">
      {item.card()}
    </Route.Link>
  })
  ...
  <div className="size-full bg-gradient-to-br from-accent/20 to-secondary/20 center">
    <div className="center flex-col gap-4 h-full p-4 max-w-7xl">
      <p className="text-5xl 2xl:text-7xl text-accent font-black inline-flex gap-4">
